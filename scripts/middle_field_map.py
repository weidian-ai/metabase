...
def insert_data():
    ...
    try:
        for data in computed_datas:
            ...
            for field in data['fields_map']:
                cursor.execute("INSERT INTO middle_field_map (db_id, table_id, e_name, c_name) values (%s, %s, \'%s\', \'%s\')" % (data['db_id'], data['table_id'], field, data['fields_map'][field]))
        print '插入成功'
        db.commit()
    except Exception as e:
        cursor.execute("INSERT INTO middle_field_map (db_id, table_id, e_name, c_name) values (%s, %s, \'%s\', \'%s\')" % (data['db_id'], data['table_id'], field, data['fields_map'][field]))
