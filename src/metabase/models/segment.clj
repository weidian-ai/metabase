...
(defn create-segment!
  ...
  [table-id db-id is-show segment-name description creator-id definition]
...
(defn update-segment!
  ...
  [{:keys [id name description is_show caveats points_of_interest show_in_getting_started definition revision_message], :as body} user-id]
  {:pre [(integer? id)
         (string? name)
         (map? definition)
         (integer? user-id)
  ...
  (db/update! Segment id
    (u/select-keys-when body
      :present #{:name :description :caveats :definition :is_show}
