...
(def ^:private ^:const builtin-geojson
  {:us_states       {:name        "United States"
   ...
   :china           {:name        "中国"
                     ...
                     :builtin     true}
   :beijing         {:name        "北京"
                     :url         "app/assets/geojson/china_beijing.json"
                     :region_key  "name"
                     :region_name "name"
                     :builtin     true}
   :shanghai        {:name        "上海"
                     :url         "app/assets/geojson/china_shanghai.json"
                     :region_key  "name"
                     :region_name "name"
                     :builtin     true}
   :guangdong       {:name        "广东"
                     :url         "app/assets/geojson/china_guangdong.json"
                     :region_key  "name"
                     :region_name "name"
                     :builtin     true}
   :shenzhen        {:name        "深圳"
                     :url         "app/assets/geojson/china_shenzhen.json"
                     :region_key  "name"
                     :region_name "name"
                     :builtin     true}
   :chengdu         {:name        "成都"
                     :url         "app/assets/geojson/china_chengdu.json"
                     :region_key  "name"
                     :region_name "name"
                     :builtin     true}})
