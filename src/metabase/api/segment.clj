...
(api/defendpoint POST "/"
  ...
  [:as {{:keys [name description table_id db_id is_show definition]} :body}]
  {name       su/NonBlankString
   table_id   su/IntGreaterThanZero
   definition su/Map}
  (api/check-superuser)
  (api/write-check Table table_id)
  (api/check-500 (segment/create-segment! table_id db_id is_show name description api/*current-user-id* definition)))
...
(api/defendpoint PUT "/:id"
  ...
  (segment/update-segment!
   (assoc (select-keys body #{:name :description :is_show :caveats :points_of_interest :show_in_getting_started :definition :revision_message})
     :id id)
   api/*current-user-id*))
