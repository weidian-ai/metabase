...
(def AnyValueLiteral
  ...
  (s/named (s/maybe (s/cond-pre s/Bool s/Str OrderableValueLiteral)) "Valid value (must be nil, boolean, number, string, or a relative-datetime form)"))
...
(s/defrecord StringFilter [filter-type :- (s/enum :starts-with :contains :ends-with :regexp-like)
                           field       :- AnyField
