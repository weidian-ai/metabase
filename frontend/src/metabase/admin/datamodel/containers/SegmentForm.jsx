...
@reduxForm({
    form: "segment",
    fields: ["id", "name", "description", "table_id", "definition", "revision_message", "is_show"],
    validate: (values) => {
        const errors = {};
        if (!values.name) {
    ...
    initialValues: { name: "", description: "", table_id: null, definition: { filter: [] }, revision_message: null, is_show: true }
},
...
export default class SegmentForm extends Component {
    ...
    componentWillReceiveProps() {
        const {fields: {is_show}} = this.props
        if(is_show && is_show['value'] !== "") {
            this.setState({
                on: is_show['value'] == 0 ? false : true
            })
        }
        return true;
    }
    render() {
        const { fields: { id, name, description, definition, revision_message, is_show }, segment, metadata, tableMetadata, handleSubmit, previewSummary } = this.props;
        const {on} = this.state
                ...
                <form className="full" onSubmit={handleSubmit}>
                    <div className="wrapper py4">
                        ...
                        <div style={{ maxWidth: "575px" }}>
                            ...
                            <FormLabel
                                title="Show in common query panel"
                                description="If true, the panel will show in common page. hide in opposite!"
                            >
                                <Toggle value={on} onChange={on => {
                                    let boolToNum = Number(on)
                                    is_show.value = boolToNum
                                    is_show.onChange(boolToNum)
