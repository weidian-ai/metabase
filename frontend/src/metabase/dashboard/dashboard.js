...
export const fetchCardData = createThunkAction(FETCH_CARD_DATA, function(card, dashcard, { reload, clear } = {}) {
    return async function(dispatch, getState) {
        ...
        if (dashboardType === "public") {
            ...
            // const cacheData = localStorage.getItem(location.href) ? JSON.parse(localStorage.getItem(location.href)) : {}
            // const hasData = Object.keys(cacheData).length > 0
            // if(hasData && cacheData[card.id]) {
            //     result = cacheData[card.id]
            // }else {
