    ...
    render() {
        ...
        if (this.props.includeTables && this.props.segments) {
            ...
            if (table) {
                ...
                content = <span className="text-grey-4 no-decoration">{getHTML('DATA_SELECTOR_SELECT_TABLE', {defaultValue: 'Select a table'})}</span>;
            }
        } else {
            if (database) {
                content = <span className="text-grey no-decoration">{database.name}</span>;
            } else {
                content = <span className="text-grey-4 no-decoration">{getHTML('DATA_SELECTOR_SELECT_DATABASE', {defaultValue: 'Select a database'})}</span>;
            }
        }
