...
import {getHTML} from "metabase/lib/intl"
    ...
    renderFilters() {
        ...
        if (query.isEditable()) {
            ...
            if (Query.canAddFilter(this.props.datasetQuery.query)) {
                addFilterButton = this.renderAdd((filterList ? null : getHTML("GUI_QUERY_EDITOR_ADD_FILTER", {defaultValue:"Add filters to narrow your answer" })), null, "addFilterTarget");
            }
        } else {
            enabled = false;
            addFilterButton = this.renderAdd(getHTML("GUI_QUERY_EDITOR_ADD_FILTER", {defaultValue:"Add filters to narrow your answer" }), null, "addFilterTarget");
        }
        ...
        for (let i = 0; i < breakouts.length; i++) {
            ...
            if (breakouts[i + 1] != null) {
                breakoutList.push(
                    ...
                    //     addButton={this.renderAdd(i === 0 ? getHTML("GUI_QUERY_EDITOR_ADD_GROUP",{defaultValue:"Add a grouping"}) : null)}
                    // />
    ...
    renderDataSection() {
        ...
        return (
            <div className={"GuiBuilder-section GuiBuilder-data flex align-center arrow-right"}>
                <span className="GuiBuilder-section-label Query-label">{getHTML("GUI_QUERY_EDITOR_DATA", {defaultValue:"Data"})}</span>
    ...
    renderFilterSection() {
        ...
        return (
            <div className="GuiBuilder-section GuiBuilder-filtered-by flex align-center" ref="filterSection">
                <span className="GuiBuilder-section-label Query-label">{getHTML("GUI_QUERY_EDITOR_FILTED", {defaultValue:"Filtered by"})}</span>
                {this.renderFilters()}
            </div>
        );
    }
    ...
    renderViewSection() {
        ...
        return (
            <div className="GuiBuilder-section GuiBuilder-view flex align-center px1 pr2" ref="viewSection">
                <span className="GuiBuilder-section-label Query-label">{getHTML("GUI_QUERY_EDITOR_VIEW", {defaultValue:"View"})}</span>
    ...
    renderGroupedBySection() {
        ...
        return (
            <div className="GuiBuilder-section GuiBuilder-groupedBy flex align-center px1" ref="viewSection">
                <span className="GuiBuilder-section-label Query-label">{getHTML("GUI_QUERY_EDITOR_GROUPED",{defaultValue:"Grouped By"})}</span>
