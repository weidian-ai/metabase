...
const selectTableByName = (db, dbName, tableName) => {
    if(!db || !dbName || !tableName) {
        return null
    }
    db = _.filter(db, (data) => data['name'] === dbName)
    if(db.length) {
        let table = _.filter(Array.isArray(db[0]['tables']) ? db[0]['tables'] : [], (table) => table['name'] === tableName)
        if(table.length) {
            return {
                db_id: table[0]['db_id'],
                id: table[0]['id']
...
export default class QueryBuilder extends Component {
    ...
    componentWillUpdate() {
        ...
        if(query && query['db'] && query['table']) {
            const {db, table} = query
            const {databases} = this.props
            const info = selectTableByName(databases, decodeURIComponent(db), decodeURIComponent(table))
            if(info) {
                const queryField = {
                    "name": null,
                    "dataset_query": {
                        ...
                        "type": "query",
                        "query": {
                            "source_table": info['id']
                        }
                    },
                    "display":"table",
                    "visualization_settings":{}
                }
                const encoded = Base64.encode(JSON.stringify(queryField))
                location.replace(`${pathname}#${encoded}`)
                location.reload()
            }
        }
    }
    ...
    render() {
        ...
        return (
            <div className="flex-full relative">
                ...
                <div className="time-down">123</div>
