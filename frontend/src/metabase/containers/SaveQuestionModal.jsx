...
export default class SaveQuestionModal extends Component {
    constructor(props, context) {
        ...
        const hasPermission = this.hasPermission = props.user && (props.user['is_superuser'] || props.user['id'] === props.card['creator_id'] )
        this.state = {
            error: null,
            valid: false,
            details: {
                ...
                saveType: props.originalCard && hasPermission ? "overwrite" : "create"
            }
        };
    }
    ...
    render() {
        ...
        if (!this.props.card.id && this.props.originalCard) {
            let optionFields = [{ name: "Save as new question", value: "create" }]
            if(this.hasPermission) {
                optionFields.unshift({ name: `Replace original question, "${this.props.originalCard.name}"`, value: "overwrite" })
            }
            saveOrUpdate = (
                <FormField
