    ...
    activityHeader(item, user) {
        ...
        switch (item.topic) {
            ...
            case "database-sync":
                ...
                break;
            case "install":
                description.userName = getHTML('HELLO_WORLD' ,{defaultValue:"Hello World!"});
                description.summary = getHTML('METABSE_IS_IP_AND_RUNNING', {defaultValue:"Metabase is up and running."});
                break;
            case "metric-create":
                if(item.model_exists) {
