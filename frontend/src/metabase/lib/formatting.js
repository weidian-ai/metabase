...
export function formatTimeWithUnit(value: Value, unit: DatetimeUnit, options: FormattingOptions = {}) {
    ...
    switch (unit) {
        case "hour": // 12 AM - January 1, 2015
            ...
            return formatMajorMinor( m.format("hA"),m.format("YYYY-MM-DD"), options)
        ...
        case "month": // January 2015
            return options.jsx ?
                <div>{m.format("YYYY")}-<span className="text-bold">{m.format("MM")}</span></div> :
                m.format("YYYY") + "-" + m.format("MM");
        case "year": // 2015
...
export function formatValue(value: Value, options: FormattingOptions = {}) {
    ...
    if (value == undefined) {
        ...
        if(column.name && column.name == 'day') {
            return parseTimestamp(value, column && column.unit).format("YYYY-MM-DD");
        }
        return parseTimestamp(value, column && column.unit).format("YYYY-MM-DD HH:mm:ss");
...
export function formatColumn(column: Column): string {
    if (!column) {
        ...
        let columnTitle = getFriendlyName(column, true);
