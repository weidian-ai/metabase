...
export default class TableInteractive extends Component {
    ...
    tableHeaderRenderer = ({ key, style, columnIndex }: CellRendererProps) => {
        ...
        if (!columnTitle && this.props.isPivoted && columnIndex !== 0) {
            ...
            let title = columnTitle.split('#_')
            columnTitle = title.splice(0, title.length - 1).join('#_')
            subTitle = title[0]
        }
        let clicked;
        if (isPivoted) {
        ...
        return (
            <div
                key={key}
                style={{ ...style, overflow: "visible", padding: 0 /* ensure resize handle is visible */ }}
            ...
            >
                <div className="cellData" style={{
                    ...
                    {columnTitle == '' ? subTitle : columnTitle}
                    {
                        columnTitle != '' &&
                        subTitle && 
                        subTitle.length > 0 &&
